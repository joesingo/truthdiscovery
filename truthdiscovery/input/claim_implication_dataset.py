...
class ClaimImplicationDataset(Dataset):
    ...
    A positive value indicates that if ``var = x`` is true, then ``var = y`` is
    likely to be true. A negative value means that if ``var = x`` is true, then
    ``var = y`` is likely to be false (Yin et.  al., 2008).
    """
    def __init__(self, sv_mat, implication_function):
        """
        :param sv_mat:               source-variables matrix as for
                                     :any:`Dataset` constructor
        :param implication_function: callback function to compute implication
                                     values. Should take ``(var, val1, val2)``
