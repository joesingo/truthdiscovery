...
For a graph $G = (V, E)$ and a permutation $\pi: V \rightarrow V$, we will
denote by $\pi(G)$ the graph isomorphic to $G$ under $\pi$. Note that $\pi(G)$
is unique.

\textbf{Conjecture}: For a truth-discovery network $N$, $\pi(N)$ is a valid
truth-discovery network if and only if $\pi(s) \in S$, $\pi(f) \in \F$ and
$\pi(o) \in \O$ for all $s, f, o$ in $N$ that have incoming or outgoing edges.
\todo{
    Think about this, and prove it if it is true
}
...
$$ s_1 \sle_N^A s_2 \iff \pi(s_1) \sle_{\pi(N)}^A \pi(s_2) $$
and
$$ f_1 \fle_N^A f_2 \iff \pi(f_1) \fle_{\pi(N)}^A \pi(f_2) $$
...
\todo{Is this an if and only if? I think yes, since if the above conjecture is
true then any permutation that results in a valid TDN can be expressed as a
composition of three mappings, each of which leaves two of $\S$, $\F$ and $\O$
unchanged (or something similar). Then applying the three symmetries to each
mapping could give overall symmetry}
