...
We consider fixed sets $\S$, $\F$ and $\O$, called the \emph{sources},
...
A \emph{truth-discovery algorithm} $A$ is a mapping $A: \mathcal{N} \rightarrow
\orderings(\S) \times \orderings(\F)$, i.e. $A$ assigns to each truth-discovery
network a pair of total preorders $A(N) = (\sle_N^A, \fle_N^A)$ on the sets
...
\begin{remark}
    Any numerical truth-discovery algorithm $A$ naturally induces a
    truth-discovery algorithm $A'$, where for any truth-discovery network $N$
    we define
    \begin{align*}
...
Let $A$ be a \tda. $A$ satisfies \emph{symmetry} if for any equivalent
truth-discovery networks $N$ and $N' = \pi(N)$, we have
$$ s_1 \sle_N^A s_2 \iff \pi(s_1) \sle_{N'}^A \pi(s_2) $$
and
$$ f_1 \fle_N^A f_2 \iff \pi(f_1) \fle_{N'}^A \pi(f_2) $$

$A$ satisfies \emph{source-symmetry} if both the above statements hold in cases
...
\begin{proposition}
$A$ satisfies symmetry if and only if it satisfies source, fact and object
symmetry.
\end{proposition}

\begin{proof}
The `only if' direction is clear. For the converse, suppose $A$ is source, fact
...
\O$. Hence $\sigma=\pi$.
% Since each of $\pi_\S$, $\pi_\F$ and $\pi_\O$ only
% permute one of $\S$, $\F$ and $\O$, we have symmetry for $A$ between any two
% networks equivalent under them.

Let $s_1, s_2 \in \S$. Since $\pi_\O$ only permutes objects, we may apply
object-symmetry to get
$$ s_1 \sle_N^A s_2 \iff \pi_\O(s_1) \sle_{\pi_\O(N)} \pi_\O(s_2) $$
...
\begin{align*}
s_1 \sle_N^A s_2 & \iff \sigma(s_1) \sle_{\sigma(N)} \sigma(s_2) \\
                 & \iff \pi(s_1) \sle_{\pi(N)} \pi(s_2) \\
                 & \iff \pi(s_1) \sle_{N'} \pi(s_2)
\end{align*}
An identical argument for fact ranking gives $f_1 \fle_N^A f_2 \iff \pi(f_1)
\fle_{N'}^A f_2$. Hence $A$ is symmetric.
...
algorithm $A$ if $s \sle_N^A s^*$ for all $s \in S$, and $(s^*, f^*) \in E$
implies $f \fle_N^A f^*$ for all $f \in \F$.
...
additionally $s \slt_N^A s^*$ for all $s \ne s^*$, and $f \flt_N^A f^*$ for all
...
\begin{axiom}
\label{axiom:dict}
An algorithm $A$ is a \emph{dictatorship} if there is a source $s^* \in S$ (the
dictator) that is authoritative for all networks.
\end{axiom}

\begin{axiom}
\label{axiom:strict_dict}
An algorithm $A$ is a \emph{strict dictatorship} if there is a source $s^* \in
S$ that is strictly authoritative for all networks.
\end{axiom}

\todo{
    Maybe should also have an axiom for algorithms that have \emph{some}
...
If an algorithm $A$ is both source-symmetric and a dictatorship, then for any
network $N$:
\begin{enumerate}
\item All sources are ranked equally
\item If $f_1$ is claimed by at least one source in $N$, then $f_2 \fle_N^A
f_1$ for all facts $f_2$.
\end{enumerate}

In particular, there is no algorithm that is both source-symmetric and a strict
dictatorship.
\end{proposition}

\begin{proof}
Suppose $A$ is source-symmetric and a dictatorship with dictator $s^*$. Let
$N=(V,E)$ be a truth-discovery network.
\begin{enumerate}
\item
    Let $s_1, s_2 \in \S$. Without loss of generality $s_1 \sle_N^A s_2$, since
    $\sle_N^A$ is complete. Consider the permutation $\pi=(s_1, s^*)$.  We have
    $s_2 \sle_{\pi(N)}^A s^*$ by dictatorship in $\pi(N)$, which by symmetry
    means $\pi^{-1}(s_2) \sle_N^A \pi^{-1}(s^*)$, i.e. $s_2 \sle_N^A s_1$.
    Hence $s_1 \seq_N^A s_2$ as required.

\item
    Let $f_1, f_2 \in \F$ such that there is a source $s$ with $(s, f_1) \in E$.
    Set $\sigma=(s, s^*)$. Then $\sigma(E)$ contains $(\sigma(s), \sigma(f_1))
    = (s^*, f_1)$, so we have $\sigma(f_2) \fle_{\sigma(N)}^A f_1 =
    \sigma(f_1)$ since the facts claimed by $s^*$ rank above all others.
    Applying symmetry gives $f_2 \fle_N^A f_1$ as required.
\end{enumerate}

This implies there is no source-symmetric strict dictatorship. If $A$ were such
an algorithm then $A$ is also a non-strict dictatorship, so symmetry implies
all sources are ranked equally, but this contradicts $s \slt_N^A s^*$ for all
$s \ne s^*$.

\end{proof}

\begin{example}
A trivial algorithm that satisfies symmetry and non-strict dictatorship is one
that always ranks all sources and facts equally: $A_{triv}(N) = (\S^2, \F^2)$.

If we restrict $\mathcal{N}$ to those networks where all facts are claimed by
at least one source, then proposition \ref{prop:symm_and_dict} shows that $A$
satisfies source-symmetry and dictatorship if and only if $A=A_{triv}$.

Without this restriction, facts not claimed by any source may be ranked
strictly below other facts. Indeed, consider $A$ defined as follows. For any
network $N$ write $F_{+} = \{f \in \F : \src(N, f) \ne \emptyset \}$, and
define $A$ by $s_1 \seq_N^{A} s_2 \text{ for all } s_1, s_2 \in \S$, and
$$
    f_1 \fle_N^{A} f_2 \iff f_2 \in F_{+} \text{ or } f_1 \notin F_{+}
    \text{ for all } f_1, f_2 \in \F
$$

$A$ is trivially a dictatorship for \emph{any} $s^* \in \S$. It can be easily
checked that $\fle_N^A$ is a well-defined total preorder, and that $A$ is also
