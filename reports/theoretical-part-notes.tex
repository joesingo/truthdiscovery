...
Note that we do not require strict inequlities here, so as to not be too
...
$T_{vote}$ is unanamous and grounded.
\end{example}
...
component $G$, the restictions of $\sle_{N_1}^T$ and $\sle_{N_2}^T$ to $G \cap
\S$ are equal, and the restictions of $\fle_{N_1}^T$ and $\fle_{N_2}^T$ to $G
\cap \F$ are equal.
\end{axiom}
