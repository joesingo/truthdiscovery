...
\begin{definition}
\label{def:gen_dict}
$T$ is a \emph{generalised dictatorship} if for every network $N$ there exists
a source $s_N \in \S$ that is authoritative for $N$ with $T$. A
\emph{generalised strict dictatorship} is defined similarly.
\end{definition}

\begin{axiom}[Non-generalised dictatorship]
\label{axiom:non_gen_dict}
An operator $T$ should not be a generalised dictatorship.
\end{axiom}
\todo{Is hyphenation above correct?}

Clearly a dictatorship is a generalised dictatorship, so axiom
\ref{axiom:non_gen_dict} implies axiom \ref{axiom:non_dict}.

\begin{example}
An example operator that is both symmetric and a generalised dictatorship is
the numeric operator defined as follows. For any truth-discovery network $N$,
...
\[
    ...
    \end{cases}
\]
Any source in $Q_N$ is authoritative. \todo{Show this operator is symmetric}
\end{example}
